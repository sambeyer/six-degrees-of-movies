...
function EmptyState() {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
      ...
      <div style={{ fontSize: 13, fontFamily: "'DM Mono', monospace", color: "var(--text-dim)", textAlign: "center", lineHeight: 1.8 }}>
...
function LoadingState() {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20 }}>
      ...
      <div style={{ fontSize: 12, fontFamily: "'DM Mono', monospace", color: "var(--text-dim)" }}>
...
export default function RightPanel({ results, loading, error, resultVersion, onClear }) {
  if (loading) return (
    <div
      ...
      role="status"
      aria-live="polite"
      aria-label="Searching for connection…"
  ...
  if (!results) return (
    <div className="right-panel" style={{ flex: 1, padding: "24px 32px", overflowY: "auto" }}>
      {error
        ? <div role="alert" style={{ fontFamily: "'DM Mono', monospace", fontSize: 12, color: "#d48080", padding: 16, background: "var(--surface)", borderRadius: 8 }}>{error}</div>
        : <EmptyState />}
    </div>
  );
    ...
    >
      <div className="fade-in-up">
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 24 }}>
          <span style={{ fontSize: 10, fontFamily: "'DM Mono', monospace", textTransform: "uppercase", letterSpacing: "0.12em", color: "var(--text-dim)" }}>
            {paths.length} path{paths.length !== 1 ? "s" : ""} found
            {results.elapsed_ms != null && (
              <span aria-hidden="true" style={{ marginLeft: 8, color: "var(--text-dim)" }}>· {results.elapsed_ms}ms</span>
            )}
          </span>
          <button
            onClick={onClear}
